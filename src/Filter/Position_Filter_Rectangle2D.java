...
public class Position_Filter_Rectangle2D implements filter{
	 ...
	 *
	 */ 
		public Position_Filter_Rectangle2D(Point3D p0, Point3D p1) {
			if(!p0.smallerThan2D(p1)) {
		...
		public boolean test(WiFi_Scan rec) {
			...
			if(rec!=null) {
				...
				if(_min.smallerThan2D(pos) && _max.biggerThan2D(pos)) {ans = true;}
			}
			return ans;
		}
